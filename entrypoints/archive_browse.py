...
def compare_run_archives(
    ...
    right_verification = _extract_mapping(right_payload["archive"], "verification_report")
    left_execution_result = _extract_mapping(left_payload["archive"], "execution_result")
    right_execution_result = _extract_mapping(right_payload["archive"], "execution_result")
    left_profile_and_mode = _extract_mapping(left_payload["archive"], "profile_and_mode")
    right_profile_and_mode = _extract_mapping(right_payload["archive"], "profile_and_mode")
    left_residual_followup = _extract_mapping(left_payload["archive"], "residual_followup")
    right_residual_followup = _extract_mapping(right_payload["archive"], "residual_followup")
    ...
    left_evaluation_summary = _extract_mapping(left_payload["archive"], "evaluation_summary")
    right_evaluation_summary = _extract_mapping(right_payload["archive"], "evaluation_summary")
    left_realm_evaluation = _extract_mapping(left_evaluation_summary, "realm_evaluation")
    right_realm_evaluation = _extract_mapping(right_evaluation_summary, "realm_evaluation")
    left_baseline_compare_results = _extract_optional_mapping(left_evaluation_summary, "baseline_compare_results")
    right_baseline_compare_results = _extract_optional_mapping(right_evaluation_summary, "baseline_compare_results")
    ...
    left_reassessment_reason_codes = _normalize_string_list(left_reassessment.get("reason_codes"))
    right_reassessment_reason_codes = _normalize_string_list(right_reassessment.get("reason_codes"))
    left_evaluation_reason_codes = _normalize_string_list(left_realm_evaluation.get("reason_codes"))
    right_evaluation_reason_codes = _normalize_string_list(right_realm_evaluation.get("reason_codes"))

    return {
        "archive_root": str(Path(archive_root)),
        "left": {
            "run_id": _normalize_optional_string(left_payload["entry"].get("run_id")) or "",
            "created_at": _normalize_optional_string(left_payload["entry"].get("created_at")) or "",
            "status": _normalize_optional_string(left_manifest.get("status")) or "",
            "workflow_profile_id": _normalize_optional_string(left_manifest.get("workflow_profile_id")) or "",
            "task_type": _normalize_optional_string(_extract_mapping(left_manifest, "task_summary").get("task_type"))
            or _normalize_optional_string(left_profile_and_mode.get("task_type"))
            or "",
            "formation_id": _normalize_optional_string(left_profile_and_mode.get("formation_id")) or "",
            **left_artifact_summary,
            "failure_class": _normalize_optional_string(left_failure.get("failure_class")) or "",
            "failed_stage": _normalize_optional_string(left_failure.get("failed_stage")) or "",
            "verification_status": _normalize_optional_string(left_verification.get("status")) or "",
            "verification_passed": bool(left_verification.get("passed")),
            "reassessed_level": _normalize_optional_string(left_reassessment.get("reassessed_level")) or "",
            "followup_needed": bool(left_reassessment.get("needs_followup")),
            "reassessment_reason_codes": left_reassessment_reason_codes,
            "evaluation_status": _normalize_optional_string(left_realm_evaluation.get("status")) or "",
            "evaluation_recommendation": _normalize_optional_string(left_realm_evaluation.get("recommendation")) or "",
            "evaluation_human_review": bool(left_realm_evaluation.get("requires_human_review")),
            "evaluation_reason_codes": left_evaluation_reason_codes,
            "governance_status": _normalize_optional_string(left_governance.get("status")) or "",
            "governance_required": bool(left_governance.get("requires_governance_override")),
            "task": _normalize_optional_string(_extract_mapping(left_manifest, "task_summary").get("task")) or "",
        },
        "right": {
            "run_id": _normalize_optional_string(right_payload["entry"].get("run_id")) or "",
            "created_at": _normalize_optional_string(right_payload["entry"].get("created_at")) or "",
            "status": _normalize_optional_string(right_manifest.get("status")) or "",
            "workflow_profile_id": _normalize_optional_string(right_manifest.get("workflow_profile_id")) or "",
            "task_type": _normalize_optional_string(_extract_mapping(right_manifest, "task_summary").get("task_type"))
            or _normalize_optional_string(right_profile_and_mode.get("task_type"))
            or "",
            "formation_id": _normalize_optional_string(right_profile_and_mode.get("formation_id")) or "",
            **right_artifact_summary,
            "failure_class": _normalize_optional_string(right_failure.get("failure_class")) or "",
            "failed_stage": _normalize_optional_string(right_failure.get("failed_stage")) or "",
            "verification_status": _normalize_optional_string(right_verification.get("status")) or "",
            "verification_passed": bool(right_verification.get("passed")),
            "reassessed_level": _normalize_optional_string(right_reassessment.get("reassessed_level")) or "",
            "followup_needed": bool(right_reassessment.get("needs_followup")),
            "reassessment_reason_codes": right_reassessment_reason_codes,
            "evaluation_status": _normalize_optional_string(right_realm_evaluation.get("status")) or "",
            "evaluation_recommendation": _normalize_optional_string(right_realm_evaluation.get("recommendation")) or "",
            "evaluation_human_review": bool(right_realm_evaluation.get("requires_human_review")),
            "evaluation_reason_codes": right_evaluation_reason_codes,
            "governance_status": _normalize_optional_string(right_governance.get("status")) or "",
            "governance_required": bool(right_governance.get("requires_governance_override")),
            "task": _normalize_optional_string(_extract_mapping(right_manifest, "task_summary").get("task")) or "",
        },
        "comparison": {
            "same_status": left_manifest.get("status") == right_manifest.get("status"),
            "same_workflow_profile_id": left_manifest.get("workflow_profile_id") == right_manifest.get("workflow_profile_id"),
            "same_task_type": _extract_mapping(left_manifest, "task_summary").get("task_type") == _extract_mapping(right_manifest, "task_summary").get("task_type"),
            "same_formation_id": left_profile_and_mode.get("formation_id") == right_profile_and_mode.get("formation_id"),
            "same_expected_artifacts": left_artifact_summary["expected_artifacts"] == right_artifact_summary["expected_artifacts"],
            "expected_artifacts_added": _list_added_items(left_artifact_summary["expected_artifacts"], right_artifact_summary["expected_artifacts"]),
            "expected_artifacts_removed": _list_removed_items(left_artifact_summary["expected_artifacts"], right_artifact_summary["expected_artifacts"]),
            "same_produced_artifact_types": left_artifact_summary["produced_artifact_types"] == right_artifact_summary["produced_artifact_types"],
            "produced_artifact_types_added": _list_added_items(left_artifact_summary["produced_artifact_types"], right_artifact_summary["produced_artifact_types"]),
            "produced_artifact_types_removed": _list_removed_items(left_artifact_summary["produced_artifact_types"], right_artifact_summary["produced_artifact_types"]),
            "same_produced_artifact_count": left_artifact_summary["produced_artifact_count"] == right_artifact_summary["produced_artifact_count"],
            "same_baseline_compare_status": left_artifact_summary["baseline_compare_status"] == right_artifact_summary["baseline_compare_status"],
            "same_baseline_compared_artifact_types": left_artifact_summary["baseline_compared_artifact_types"] == right_artifact_summary["baseline_compared_artifact_types"],
            "baseline_compared_artifact_types_added": _list_added_items(left_artifact_summary["baseline_compared_artifact_types"], right_artifact_summary["baseline_compared_artifact_types"]),
            "baseline_compared_artifact_types_removed": _list_removed_items(left_artifact_summary["baseline_compared_artifact_types"], right_artifact_summary["baseline_compared_artifact_types"]),
            "same_baseline_status_counts": left_artifact_summary["baseline_status_counts"] == right_artifact_summary["baseline_status_counts"],
            "same_missing_expected_artifact_warning": left_artifact_summary["missing_expected_artifact_warning"] == right_artifact_summary["missing_expected_artifact_warning"],
            "same_failure_class": left_failure.get("failure_class") == right_failure.get("failure_class"),
            "same_failed_stage": left_failure.get("failed_stage") == right_failure.get("failed_stage"),
            "same_verification_status": left_verification.get("status") == right_verification.get("status"),
            "same_reassessed_level": left_reassessment.get("reassessed_level") == right_reassessment.get("reassessed_level"),
            "same_followup_needed": bool(left_reassessment.get("needs_followup")) == bool(right_reassessment.get("needs_followup")),
            "same_reassessment_reason_codes": left_reassessment_reason_codes == right_reassessment_reason_codes,
            "reassessment_reason_codes_added": _list_added_items(left_reassessment_reason_codes, right_reassessment_reason_codes),
            "reassessment_reason_codes_removed": _list_removed_items(left_reassessment_reason_codes, right_reassessment_reason_codes),
            "same_evaluation_status": left_realm_evaluation.get("status") == right_realm_evaluation.get("status"),
            "same_evaluation_recommendation": left_realm_evaluation.get("recommendation") == right_realm_evaluation.get("recommendation"),
            "same_evaluation_human_review": bool(left_realm_evaluation.get("requires_human_review")) == bool(right_realm_evaluation.get("requires_human_review")),
            "same_evaluation_reason_codes": left_evaluation_reason_codes == right_evaluation_reason_codes,
            "evaluation_reason_codes_added": _list_added_items(left_evaluation_reason_codes, right_evaluation_reason_codes),
            "evaluation_reason_codes_removed": _list_removed_items(left_evaluation_reason_codes, right_evaluation_reason_codes),
            "same_governance_status": left_governance.get("status") == right_governance.get("status"),
            "same_governance_required": bool(left_governance.get("requires_governance_override")) == bool(right_governance.get("requires_governance_override")),
            "same_task": _extract_mapping(left_manifest, "task_summary").get("task") == _extract_mapping(right_manifest, "task_summary").get("task"),
            "failure_transition": _classify_failure_transition(left_manifest, right_manifest, left_failure, right_failure),
            ...
            "evaluation_transition": _classify_evaluation_transition(left_realm_evaluation, right_realm_evaluation),
            ...
            "artifact_transition": _classify_artifact_transition(left_artifact_summary, right_artifact_summary),
        },
    }
...
def _read_archive_record(archive_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "verification_report": _read_json_mapping(archive_dir / "verification_report.json"),
        "execution_result": _extract_optional_mapping(final_output, "execution_result"),
        "residual_followup": _extract_optional_mapping(
            final_output,
            "residual_followup",
        ),
        "evaluation_summary": _read_json_mapping(archive_dir / "evaluation_summary.json"),
...
def _merge_archive_entries(
    ...
    merged_entries.sort(key=lambda item: _normalize_optional_string(item.get("created_at")) or "")
...
def _hydrate_archive_index_entry(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    manifest = _read_json_mapping(archive_dir / "manifest.json") if (archive_dir / "manifest.json").exists() else {}
    profile_and_mode = _read_json_mapping(archive_dir / "profile_and_mode.json") if (archive_dir / "profile_and_mode.json").exists() else {}
    failure_signature = _read_json_mapping(archive_dir / "failure_signature.json") if (archive_dir / "failure_signature.json").exists() else {}
    verification_report = _read_json_mapping(archive_dir / "verification_report.json") if (archive_dir / "verification_report.json").exists() else {}
    final_output = _read_json_mapping(archive_dir / "final_output.json") if (archive_dir / "final_output.json").exists() else {}
    task_summary = _extract_optional_mapping(manifest, "task_summary")
    governance = _extract_optional_mapping(_extract_optional_mapping(final_output, "residual_followup"), "governance")
    ...
    if "formation_id" not in hydrated:
        hydrated["formation_id"] = _normalize_optional_string(profile_and_mode.get("formation_id")) or ""
    if "policy_mode" not in hydrated:
        hydrated["policy_mode"] = _normalize_optional_string(profile_and_mode.get("policy_mode")) or ""
    if "workflow_profile_id" not in hydrated:
        hydrated["workflow_profile_id"] = _normalize_optional_string(manifest.get("workflow_profile_id")) or ""
    ...
    if "failure_class" not in hydrated:
        hydrated["failure_class"] = _normalize_optional_string(failure_signature.get("failure_class"))
    if "governance_status" not in hydrated:
        hydrated["governance_status"] = _normalize_optional_string(governance.get("status")) or ""
    if "governance_required" not in hydrated:
        hydrated["governance_required"] = bool(governance.get("requires_governance_override"))
    if "missing_expected_artifact_warning" not in hydrated:
        hydrated["missing_expected_artifact_warning"] = "missing_expected_artifact" in warning_codes
    return hydrated
...
def _scan_archive_dirs(archive_root: Path) -> list[dict[str, Any]]:
    ...
    for path in archive_root.iterdir():
        ...
        profile_and_mode = _read_json_mapping(profile_path) if profile_path.exists() else {}
        verification_report = _read_json_mapping(path / "verification_report.json") if (path / "verification_report.json").exists() else {}
        final_output = _read_json_mapping(path / "final_output.json") if (path / "final_output.json").exists() else {}
        governance = _extract_optional_mapping(_extract_optional_mapping(final_output, "residual_followup"), "governance")
        entries.append(
            {
                "run_id": _normalize_optional_string(manifest.get("run_id")) or path.name,
                "created_at": _normalize_optional_string(manifest.get("created_at")) or "",
                "workflow_profile_id": _normalize_optional_string(manifest.get("workflow_profile_id")) or "",
                "task_type": _normalize_optional_string(_extract_mapping(manifest, "task_summary").get("task_type"))
                or _normalize_optional_string(profile_and_mode.get("task_type"))
                or "",
                "formation_id": _normalize_optional_string(profile_and_mode.get("formation_id")) or "",
                "policy_mode": _normalize_optional_string(profile_and_mode.get("policy_mode")) or "",
                ...
                "governance_status": _normalize_optional_string(governance.get("status")) or "",
                "governance_required": bool(governance.get("requires_governance_override")),
                "missing_expected_artifact_warning": "missing_expected_artifact" in _verification_warning_codes(verification_report),
            }
        )
...
def _filter_archive_entries(
    ...
    *,
    ...
    for entry in entries:
        if workflow_profile_id_text and _normalize_optional_string(entry.get("workflow_profile_id")) != workflow_profile_id_text:
            continue
        if task_type_text and _normalize_optional_string(entry.get("task_type")) != task_type_text:
            continue
        if formation_id_text and _normalize_optional_string(entry.get("formation_id")) != formation_id_text:
            continue
        if status_text and _normalize_optional_string(entry.get("status")) != status_text:
            continue
        if failure_class_text and _normalize_optional_string(entry.get("failure_class")) != failure_class_text:
            continue
...
def _format_archive_entry_payload(payload: Mapping[str, Any]) -> str:
    ...
    governance = _extract_mapping(_extract_mapping(archive, "residual_followup"), "governance")
    baseline_compare_results = _extract_optional_mapping(_extract_mapping(archive, "evaluation_summary"), "baseline_compare_results")
...
def _classify_failure_transition(
    ...
    left_failure_class = _normalize_optional_string(left_failure.get("failure_class")) or ""
    right_failure_class = _normalize_optional_string(right_failure.get("failure_class")) or ""
...
def _classify_risk_transition(
    ...
    if left_normalized == right_normalized:
        return "unchanged"
    if left_normalized not in RISK_LEVEL_ORDER or right_normalized not in RISK_LEVEL_ORDER:
        return "changed"
...
def _classify_evaluation_transition(
    ...
    left_recommendation = _normalize_optional_string(left_evaluation.get("recommendation")) or ""
    right_recommendation = _normalize_optional_string(right_evaluation.get("recommendation")) or ""

    if left_requires_review == right_requires_review and left_recommendation == right_recommendation:
        return "unchanged"
...
def _build_artifact_summary(
    *,
    ...
    warning_codes = _verification_warning_codes(verification_report)
    return {
        "expected_artifacts": _normalize_string_list(task_contract.get("expected_artifacts")),
        "produced_artifact_types": produced_artifact_types,
        "produced_artifact_count": len(artifacts),
        "baseline_compare_status": _normalize_optional_string(baseline_compare_results.get("status")) or "",
        "baseline_compared_artifact_types": _normalize_string_list(baseline_compare_results.get("compared_artifact_types")),
        "baseline_status_counts": _normalize_status_counts(baseline_compare_results.get("status_counts")),
        "missing_expected_artifact_warning": "missing_expected_artifact" in warning_codes,
    }
...
def _classify_artifact_transition(
    ...
    left_score = _artifact_status_score(_extract_optional_mapping(left_summary, "baseline_status_counts"))
    right_score = _artifact_status_score(_extract_optional_mapping(right_summary, "baseline_status_counts"))
