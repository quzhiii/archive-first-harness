...
\author{Anonymous \\ Anonymous University \\ \texttt{anonymous@example.com}}
...
This paper makes four contributions: (\textbf{1})~a conceptual articulation of the \textbf{Agent Evidence Layer} as a missing layer distinct from frameworks, harnesses, and observability systems; (\textbf{2})~a structured archive schema and stage-aware comparison model; (\textbf{3})~a zero-dependency local CLI implementation; and (\textbf{4})~empirical evidence through AgentRx failure-to-stage mapping, deterministic case-study pipelines, information-density quantification, and a blind LLM proxy evaluation.
...
The LLM proxy evaluation surfaces a \textbf{stage label ambiguity}: for Scenario 1, both frontier models consistently classify the failure stage as \texttt{execution} rather than \texttt{verification} when reading the archive compare output (0/6 correct). The underlying cause is that retrieval strategy degradation is an execution-time behavior, yet the archive schema surfaces it through the verification report. Stage labels for retrieval-specific failures may benefit from an additional sub-label in future schema versions; root cause identification is unaffected.
