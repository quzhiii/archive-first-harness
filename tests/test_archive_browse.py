...
class ArchiveBrowseTests(unittest.TestCase):
    ...
    def test_browse_run_archives_filters_recent_rows(self) -> None:
        self._write_success_archive(run_id="run-success-a", task_type="research", minutes_offset=0, formation_id="discovery")
        self._write_failed_archive(run_id="run-failed-review", task_type="review", minutes_offset=1, formation_id="review")
        self._write_success_archive(run_id="run-success-b", task_type="research", minutes_offset=2, formation_id="delivery")
        ...
        self.assertIn("filters: workflow_profile_id=evaluation_regression task_type=review formation_id=review status=failed failure_class=surface_execution_failure", output)
        self.assertIn("task_type=review | formation=review | status=failed | failure=surface_execution_failure | governance=clear | gov_required=no | missing_expected=no", output)

    def test_browse_summary_surfaces_governance_and_missing_expected_warning(self) -> None:
        self._write_governance_review_archive(run_id="run-governance-review", task_type="research", minutes_offset=0)
        self._write_missing_expected_artifact_archive(run_id="run-missing-artifact", task_type="coding", minutes_offset=1)
        ...
        self.assertIn("governance=review_required | gov_required=yes | missing_expected=no", governance_output)
        self.assertIn("governance=clear | gov_required=no | missing_expected=yes", warning_output)

    def test_read_latest_run_archive_prefers_index_and_falls_back_to_directory_scan(self) -> None:
        self._write_success_archive(run_id="run-success-a", task_type="research", minutes_offset=0, formation_id="discovery")
        self._write_failed_archive(run_id="run-failed-review", task_type="review", minutes_offset=1, formation_id="review")

        latest_payload = read_latest_run_archive(self.archive_root)
        self.assertEqual(latest_payload["source"], "index_file")
        self.assertEqual(latest_payload["latest_archive"]["run_id"], "run-failed-review")

        (self.archive_root / "index.jsonl").unlink()

        fallback_latest = read_latest_run_archive(self.archive_root)
        fallback_browse = browse_run_archives(self.archive_root, limit=5, status="failed")

        self.assertEqual(fallback_latest["source"], "archive_dirs")
        self.assertEqual(fallback_latest["latest_archive"]["run_id"], "run-failed-review")
        self.assertEqual(fallback_browse["source"], "archive_dirs")
        self.assertEqual([item["run_id"] for item in fallback_browse["entries"]], ["run-failed-review"])

    def test_find_run_archive_returns_minimal_diagnostic_bundle(self) -> None:
        self._write_success_archive(run_id="run-success-a", task_type="research", minutes_offset=0)
    ...
    def test_compare_run_archives_surfaces_reassessment_and_evaluation_changes(self) -> None:
        self._write_success_archive(run_id="run-success-a", task_type="research", minutes_offset=0, formation_id="discovery")
        self._write_failed_archive(run_id="run-failed-review", task_type="review", minutes_offset=1, formation_id="review")

        payload = compare_run_archives(self.archive_root, "run-success-a", "run-failed-review")
        output = format_archive_brief(payload)

        self.assertNotEqual(payload["left"]["reassessed_level"], payload["right"]["reassessed_level"])
        ...
        self.assertEqual(payload["left"]["reassessment_reason_codes"], ["execution_clean"])
        self.assertEqual(payload["right"]["reassessment_reason_codes"], ["execution_failed", "verification_issues_present"])
        self.assertEqual(payload["left"]["evaluation_reason_codes"], ["stable_baseline"])
        self.assertEqual(payload["right"]["evaluation_reason_codes"], ["execution_failure_detected"])
        ...
        self.assertEqual(payload["comparison"]["reassessment_reason_codes_added"], ["execution_failed", "verification_issues_present"])
        self.assertEqual(payload["comparison"]["reassessment_reason_codes_removed"], ["execution_clean"])
        ...
        self.assertEqual(payload["comparison"]["evaluation_reason_codes_added"], ["execution_failure_detected"])
        self.assertEqual(payload["comparison"]["evaluation_reason_codes_removed"], ["stable_baseline"])
        ...
        self.assertIn("transitions: failure=regressed verification=regressed reassessment=regressed evaluation=regressed governance=unchanged artifacts=changed", output)
        self.assertIn("artifact_diff: transition=changed expected(+audit_note; -report) status_counts(left=none right=none) missing_expected=no->no", output)
        self.assertNotIn("produced(+none; -none)", output)
        self.assertNotIn("baseline(+none; -none)", output)
        self.assertIn("reason_code_diff: reassessment(+execution_failed,verification_issues_present; -execution_clean) evaluation(+execution_failure_detected; -stable_baseline)", output)
        self.assertIn("highlights: failure regressed; verification regressed; risk regressed; evaluation regressed; artifacts changed; reassessment reasons +execution_failed,verification_issues_present -execution_clean; evaluation reasons +execution_failure_detected -stable_baseline; expected artifacts +audit_note -report", output)

    def test_compare_run_archives_surfaces_governance_changes(self) -> None:
        self._write_success_archive(run_id="run-success-a", task_type="research", minutes_offset=0)
        self._write_governance_review_archive(run_id="run-governance-review", task_type="research", minutes_offset=1)

        payload = compare_run_archives(self.archive_root, "run-success-a", "run-governance-review")
        output = format_archive_brief(payload)
        ...
        self.assertIn("Archive comparison", output)
        self.assertIn("reason_code_diff: reassessment(+none; -none) evaluation(+none; -none)", output)
        self.assertIn("highlights: failure regressed; governance escalated", output)
        self.assertIn("transitions: failure=regressed verification=unchanged reassessment=unchanged evaluation=unchanged governance=escalated", output)

    def test_compare_run_archives_surfaces_artifact_summary_changes(self) -> None:
        self._write_success_archive(run_id="run-success-a", task_type="research", minutes_offset=0, formation_id="discovery")
        self._write_artifact_signal_archive(run_id="run-artifact-signal", task_type="research", minutes_offset=1, formation_id="discovery")

        payload = compare_run_archives(self.archive_root, "run-success-a", "run-artifact-signal")
        output = format_archive_brief(payload)
        ...
        self.assertEqual(payload["right"]["baseline_compared_artifact_types"], ["metrics_summary", "verification_report"])
        self.assertEqual(payload["left"]["baseline_status_counts"], {})
        self.assertEqual(payload["right"]["baseline_status_counts"], {"compatible": 1, "warning": 1})
        ...
        self.assertEqual(payload["comparison"]["expected_artifacts_added"], ["code_patch"])
        self.assertEqual(payload["comparison"]["expected_artifacts_removed"], ["report"])
        self.assertEqual(payload["comparison"]["produced_artifact_types_added"], ["file_change"])
        self.assertEqual(payload["comparison"]["produced_artifact_types_removed"], [])
        self.assertEqual(payload["comparison"]["baseline_compared_artifact_types_added"], ["metrics_summary", "verification_report"])
        self.assertEqual(payload["comparison"]["baseline_compared_artifact_types_removed"], [])
        self.assertEqual(payload["comparison"]["artifact_transition"], "regressed")
        self.assertIn("transitions: failure=unchanged verification=unchanged reassessment=unchanged evaluation=unchanged governance=unchanged artifacts=regressed", output)
        self.assertIn("artifacts_left: expected=report produced=none(0) baseline_status=not_requested baseline_artifacts=none status_counts=none missing_expected=no", output)
        self.assertIn("artifacts_right: expected=code_patch produced=file_change(1) baseline_status=completed baseline_artifacts=metrics_summary,verification_report status_counts=compatible:1,warning:1 missing_expected=no", output)
        self.assertIn("artifact_diff: transition=regressed expected(+code_patch; -report) produced(+file_change; -none) baseline(+metrics_summary,verification_report; -none) status_counts(left=none right=compatible:1,warning:1) missing_expected=no->no", output)
        self.assertIn("highlights: artifacts regressed; expected artifacts +code_patch -report; produced artifacts +file_change; baseline artifacts +metrics_summary,verification_report", output)

    def test_archive_browse_tolerates_malformed_index_lines(self) -> None:
        self._write_success_archive(run_id="run-success-a", task_type="research", minutes_offset=0)
        self._write_success_archive(run_id="run-success-b", task_type="research", minutes_offset=1)
        index_file = self.archive_root / "index.jsonl"
        index_file.write_text(index_file.read_text(encoding="utf-8") + "}\n", encoding="utf-8")
        ...
        self.assertEqual([item["run_id"] for item in browse_payload["entries"]], ["run-success-a", "run-success-b"])
    ...
    def test_cli_archive_latest_lookup_and_compare_are_human_readable(self) -> None:
        self._write_success_archive(run_id="run-success-a", task_type="research", minutes_offset=0, formation_id="discovery")
        self._write_governance_review_archive(run_id="run-governance-review", task_type="research", minutes_offset=1, formation_id="review")
        ...
        self.assertIn("task_type=research formation_id=discovery", browse_stdout.getvalue())
        self.assertIn("task_type=research | formation=discovery", browse_stdout.getvalue())
        ...
        self.assertIn("highlights: failure regressed; governance escalated", compare_stdout.getvalue())
        self.assertNotIn("{", compare_stdout.getvalue())

    def test_archive_browse_is_read_only_and_does_not_mutate_index(self) -> None:
        self._write_success_archive(run_id="run-success-a", task_type="research", minutes_offset=0)
        self._write_failed_archive(run_id="run-failed-review", task_type="review", minutes_offset=1)
        index_file = self.archive_root / "index.jsonl"
        ...
        self.assertEqual((archive_dir / "manifest.json").read_text(encoding="utf-8"), before_manifest)
    ...
    def _write_success_archive(self, *, run_id: str, task_type: str, minutes_offset: int, formation_id: str = "default") -> None:
    ...
    def _write_failed_archive(self, *, run_id: str, task_type: str, minutes_offset: int, formation_id: str = "default") -> None:
        result = run_task_request(
            SurfaceTaskRequest(task=f"Task for {run_id}", task_type=task_type),
            load_settings(),
    ...
    def _write_governance_review_archive(self, *, run_id: str, task_type: str, minutes_offset: int, formation_id: str = "default") -> None:
        result = run_task_request(
            SurfaceTaskRequest(task=f"Task for {run_id}", task_type=task_type),
            load_settings(),
        )
    ...
    def _write_artifact_signal_archive(self, *, run_id: str, task_type: str, minutes_offset: int, formation_id: str = "default") -> None:
        result = run_task_request(
            SurfaceTaskRequest(task=f"Task for {run_id}", task_type=task_type),
            load_settings(),
        )
        ...
        modified_result["baseline_compare_results"] = {
            ...
            "artifact_results": {
                "verification_report": {"artifact_type": "verification_report", "status": "compatible"},
                "metrics_summary": {"artifact_type": "metrics_summary", "status": "warning"},
            },
    ...
    def _write_missing_expected_artifact_archive(self, *, run_id: str, task_type: str, minutes_offset: int, formation_id: str = "default") -> None:
        result = run_task_request(
            SurfaceTaskRequest(task=f"Task for {run_id}", task_type=task_type),
            load_settings(),
        )
